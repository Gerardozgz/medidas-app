...
const INLINE_JSON_KEY = "gs_inline_json"; // 👈 vuelta para pruebas locales
const APIKEY_KEY = "gs_api_key"; // 🔑 clave opcional para proteger el backend
...
async function loadEndpointStrict(endpoint: string, key?: string): Promise<{ clases: Clase[]; medidas: { id: string; nombre: string; descripcion?: string }[] }>{
  ...
  try {
    ...
    // 2) Fallback JSONP (necesita soporte de callback en Apps Script)
...
// ==========================================
// Componentes UI (sin libs externas)
// ==========================================
  ...
  return (
    <div className="fixed inset-0 z-50">
      ...
      <div className="absolute inset-0 flex items-center justify-center p-4">
        <div className={`w-full ${maxWidth} bg-white rounded-xl shadow-xl`}>
...
function SelectorMedidas({ medidas, onPick }: {
  medidas: { id: string; nombre: string; descripcion?: string }[];
    ...
    <div className="space-y-3">
      ...
      <div className="h-[50vh] overflow-auto pr-2">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          ...
          {filtradas.length === 0 && <div className="col-span-full"><EmptyState title="Sin resultados" subtitle="Ajusta tu búsqueda" icon="⚙️" /></div>}
        </div>
      </div>
    </div>
  );
...
export default function AppMedidas() {
  ...
  const clase = useMemo(
    () => clases.find((c) => c.id === selectedClassId) ?? clases[0],
    [clases, selectedClassId]
  );
  useEffect(() => {
    if (clases.length)
      setSelectedClassId((prev) =>
        clases.some((c) => c.id === prev) ? prev : clases[0].id
      );
  }, [clases]);
  ...
  const [estado, setEstado] = usePersistedState<Record<string, EstadoAlumno>>(
    STORAGE_KEY,
    {}
  );
  const getAlumnoEstado = (id: string) => estado[id] ?? { alumnoId: id, medidas: [] };
  const upsertAlumnoEstado = (
    alumnoId: string,
    updater: (e: EstadoAlumno) => EstadoAlumno
  ) =>
    setEstado((prev) => {
  ...
  const addComentario = (alumnoId: string, medidaId: string, texto: string) =>
    upsertAlumnoEstado(alumnoId, (e) =>
      applyAddComentario(e, medidaId, texto, ahoraISO, uuid)
    );
    ...
    const load = async () => {
      ...
      try {
        ...
        if (endpoint) {
          ...
          // intentar cargar estado compartido (si el backend lo expone)
          try {
      ...
      // Fallback final: pegar JSON manual
      try {
        if (inlineJSON) {
  ...
  const printAlumno = (al: { id: string; nombre: string }) => {
    ...
    const html = buildAlumnoReportHTML(
      clase?.nombre || "",
      al.nombre,
      est,
      fmtFecha
  ...
  const printClase = () => {
    if (!clase) return;
    const html = buildClaseReportHTML(
      clase.nombre,
      clase.alumnos,
      estado,
      fmtFecha
  ...
  const [showSelectorForAlumno, setShowSelectorForAlumno] = useState<string | null>(
    null
  );
    ...
    <div className="min-h-screen bg-gradient-to-b from-white to-slate-50 p-4 sm:p-8">
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Selecciona una clase, añade medidas a cada alumno y registra comentarios de seguimiento.
            </p>
          </div>
          <div className="flex flex-col sm:flex-row sm:items-center gap-2">
            <ReportMenu onPrint={printClase} onDownload={downloadClase} label="Informe de clase" />
            <button
              className="border rounded-2xl px-3 py-2 text-sm hover:bg-slate-50"
              onClick={() => setShowFuente((v) => !v)}
            >
              ⚙️ Fuente de datos
            </button>
            <div className="flex items-center gap-2">
              ...
              {usandoPegado && (
                <span className="text-xs rounded-full bg-amber-100 text-amber-900 px-2 py-1" title="Mostrando datos pegados en local">
                  Fuente: JSON pegado
                </span>
              )}
            </div>
        ...
        {showFuente && (
          <div className="border rounded-xl p-4 space-y-3">
            ...
            <div className="text-sm text-muted-foreground">
              Pega la URL de tu Apps Script desplegado (termina en <code>/exec</code>). Si tu navegador bloquea CORS, la app intentará JSONP. Si tu entorno no permite JSONP (CSP), puedes <strong>pegar</strong> el JSON aquí abajo para probar.
            </div>
            <div className="flex items-center gap-2">
              <input
                className="w-full border rounded-lg px-3 py-2"
                placeholder="https://script.google.com/macros/s/DEPLOY_ID/exec"
                value={endpoint}
              ...
              {endpoint && (
                <div className="flex items-center gap-2">
                  ...
                  {apiKey && (
                    <a className="text-sm underline" href={`${endpoint}${endpoint.includes('?') ? '&' : '?'}key=${encodeURIComponent(apiKey)}`} target="_blank" rel="noreferrer">
                      Abrir con key
                    </a>
                  )}
                </div>
            ...
            <textarea
              className="w-full min-h-[140px] border rounded-lg p-2 font-mono text-xs"
              placeholder='{"medidas":[{"id":"M1","nombre":"Medida"}],"clases":[{"id":"1A","nombre":"1ºA","alumnos":[{"id":"A1","nombre":"Ana"}]}]}'
              value={inlineJSON}
                ...
                <div className="p-4 border-b">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <ReportMenu
                        onPrint={() => printAlumno(al)}
                        onDownload={() => downloadAlumno(al)}
                        label="Informe"
                      />
                      <button
                        className="rounded-full bg-slate-900 text-white text-sm px-3 py-2"
                        onClick={() => setShowSelectorForAlumno(al.id)}
                      >
                        ➕ Añadir medida
                      </button>
                    </div>
                          ...
                          <div key={m.id} className="border rounded-xl">
                            <div className="p-3 border-b">
                              <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-2">
                                  <ComentarioButton
                                    onSubmit={(texto) => { try { if (endpoint) postAccion(endpoint, { action:'comentar', alumno_id: al.id, alumno_nombre: al.nombre, medida_id: m.id, medida_nombre: m.nombre, texto }, apiKey); } catch {} addComentario(al.id, m.id, texto); }}
                                  />
                                  <button
                                    className="border rounded-full px-2.5 py-2 text-sm"
                                    title="Desactivar medida"
                                    onClick={() => { try { if (endpoint) postAccion(endpoint, { action:'toggle', alumno_id: al.id, alumno_nombre: al.nombre, medida_id: m.id, medida_nombre: m.nombre }, apiKey); } catch {} toggleActiva(al.id, m.id); }}
                                  >
                                    ⏻
                                  </button>
                                </div>
                            ...
                            <div key={m.id} className="border rounded-lg p-3">
                              <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-2">
                                  ...
                                  />
                                  <button
                                    className="rounded-full px-3 py-2 text-sm hover:bg-slate-50"
                                    onClick={() => { try { if (endpoint) postAccion(endpoint, { action:'toggle', alumno_id: al.id, alumno_nombre: al.nombre, medida_id: m.id, medida_nombre: m.nombre }, apiKey); } catch {} toggleActiva(al.id, m.id); }}
                                  >
                                    Reactivar
                                  </button>
        ...
        {alumnosFiltrados.length === 0 && (
          <EmptyState title="Sin alumnos" subtitle="Prueba a limpiar el filtro de búsqueda" />
        )}
      </div>

      {/* Modal de selección de medidas */}
      <Modal
        open={!!showSelectorForAlumno}
        onClose={() => setShowSelectorForAlumno(null)}
        title="Selecciona una medida"
        description={
          showSelectorForAlumno
            ? `Se activará para el alumno seleccionado.`
            : undefined
        }
      >
        <SelectorMedidas
          ...
          onPick={(m) => {
            if (showSelectorForAlumno) {
              try {
                if (endpoint) {
                  const al = clase?.alumnos.find(a => a.id === showSelectorForAlumno);
                  postAccion(endpoint, { action:'activar', alumno_id: showSelectorForAlumno, alumno_nombre: al?.nombre || '', medida_id: m.id, medida_nombre: m.nombre }, apiKey).catch(console.warn);
                }
              } catch {}
...
// ==========================================
// Mini tests (muy ligeros)
// ==========================================
function __runMiniTests(){
  try{
    const N=()=>'2025-01-01T00:00:00.000Z';
    let e:EstadoAlumno={alumnoId:'T',medidas:[]};
    e=applyActivarMedida(e,{id:'M',nombre:'Medida'},N); console.assert(e.medidas.length===1 && e.medidas[0].activa,'activar');
    e=applyAddComentario(e,'M','ok',N,()=> 'c1'); console.assert(e.medidas[0].comentarios.length===1,'coment');
    e=applyToggleActiva(e,'M',N); console.assert(e.medidas[0].activa===false,'toggle_off');
    e=applyToggleActiva(e,'M',N); console.assert(e.medidas[0].activa===true,'toggle_on');
    const html=buildAlumnoReportHTML('CL','AL',e,()=> 'D'); console.assert(html.startsWith('<!doctype html>'),'doctype');
  }catch(err){console.warn('mini tests warning',err);} }
if(typeof window!=="undefined" && !(window as any).__APP_MINITEST__){ (window as any).__APP_MINITEST__=true; __runMiniTests(); }
